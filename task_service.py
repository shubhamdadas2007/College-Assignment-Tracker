...
Enforces role-based completion workflow, student isolation, validation, filtering, search, sorting, and dashboard metrics.
"""
import sqlite3
...
class TaskService:
    ...
    def create_task(
        ...
        Enforces permissions: Students can only create tasks for themselves and cannot set Completed/Needs Resubmission directly.
        """
        if acting_user and acting_user.is_student:
            if acting_user.id != student_id:
                return False, None, "Permission denied: Students can only create tasks for themselves."
            if status in ("Completed", "Needs Resubmission"):
                return False, None, "Permission denied: Students cannot set task status directly to Completed or Needs Resubmission."
    ...
    def update_task(
        ...
        Enforces permissions: Students can only edit their own tasks and cannot set Completed or Needs Resubmission directly.
        """
        ...
        if acting_user and acting_user.is_student:
            ...
            if status == "Completed" and existing.status != "Completed":
                return False, "Permission denied: Students cannot mark tasks as Completed. Please submit for verification."
            if status == "Needs Resubmission" and existing.status != "Needs Resubmission":
                return False, "Permission denied: Only Administrators/Teachers can set status to Needs Resubmission."
    ...
    def update_task_status(self, task_id: int, new_status: str, acting_user: Optional[User] = None) -> Tuple[bool, str]:
        """
        Update the status of a task with strict role-based completion workflow enforcement.
        - Student: Can transition Pending / In Progress / Needs Resubmission -> Submitted.
                   CANNOT set Completed or Needs Resubmission.
        - Admin: Can approve (Submitted -> Completed) or reject (Submitted -> Needs Resubmission).
        """
        ...
        # Permission checks for Student
        if acting_user and acting_user.is_student:
            if existing.student_id != acting_user.id:
                return False, "Permission denied: You can only update your own tasks."

            if new_status == "Completed":
                return False, "Permission denied: Students cannot mark tasks as Completed. Please submit for verification."

            if new_status == "Needs Resubmission":
                return False, "Permission denied: Only Administrators/Teachers can request resubmission."

            if new_status == "Submitted":
                if existing.status == "Completed":
                    return False, "This task is already verified and marked as Completed."
                if existing.status == "Submitted":
                    return False, "This task is already submitted and awaiting verification."
        ...
        try:
            ...
            if new_status == "Submitted":
                return True, "Task submitted for verification 📤."
            elif new_status == "Completed":
                return True, "Task submission approved and marked as Completed ✅."
            elif new_status == "Needs Resubmission":
                return True, "Resubmission requested from student ↻."
            else:
    ...
    def get_dashboard_metrics(self, student_id: Optional[int] = None, current_date: Optional[date] = None) -> Dict[str, Any]:
        """
        Calculate live statistics directly from SQLite for Dashboards.
        Returns counts for: Total, Pending, In Progress, Submitted, Needs Resubmission, Completed, Overdue, Due Today.
        Strictly counts Completed only for teacher/admin approved tasks.
        """
        ...
        try:
            ...
            # 2. Completed (Teacher/Admin Approved)
            cursor.execute(
            ...
            # 3. Submitted (Awaiting Verification)
            cursor.execute(
                f"SELECT COUNT(*) FROM tasks {where_student} {'AND' if where_student else 'WHERE'} status = 'Submitted';",
                params
            )
            submitted = cursor.fetchone()[0]

            # 4. Needs Resubmission
            cursor.execute(
                f"SELECT COUNT(*) FROM tasks {where_student} {'AND' if where_student else 'WHERE'} status = 'Needs Resubmission';",
                params
            )
            needs_resubmission = cursor.fetchone()[0]
            ...
            # 7. Overdue (status NOT IN ('Completed', 'Submitted') AND due_date < today)
            cursor.execute(
                f"SELECT COUNT(*) FROM tasks {where_student} {'AND' if where_student else 'WHERE'} status NOT IN ('Completed', 'Submitted') AND due_date < ?;",
                params + [today_str]
            )
            overdue = cursor.fetchone()[0]

            # 8. Due Today (status NOT IN ('Completed', 'Submitted') AND due_date == today)
            cursor.execute(
                f"SELECT COUNT(*) FROM tasks {where_student} {'AND' if where_student else 'WHERE'} status NOT IN ('Completed', 'Submitted') AND due_date = ?;",
                params + [today_str]
            )
    ...
    def get_upcoming_deadlines(self, student_id: Optional[int] = None, limit: int = 8, current_date: Optional[date] = None) -> List[Task]:
        """
    ...
    def get_overdue_students(self, current_date: Optional[date] = None) -> List[Dict[str, Any]]:
        ...
        (excluding Completed and Submitted tasks).
        """
        today = current_date or get_current_date()
        today_str = today.strftime("%Y-%m-%d")
        ...
        try:
            cursor.execute("""
                ...
                WHERE u.role = 'Student' AND t.status NOT IN ('Completed', 'Submitted') AND t.due_date < ?
    ...
    def get_subject_workload(self, student_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        Strictly calculates completion rate based on Completed tasks.
        """
        conn = get_connection(self.db_path)
        cursor = conn.cursor()
        try:
            ...
            query = f"""
                SELECT 
                    ...
                    SUM(CASE WHEN t.status = 'Needs Resubmission' THEN 1 ELSE 0 END) AS resubmission_tasks,
            ...
            for r in rows:
                ...
                result.append({
                    ...
                    "submitted_tasks": r["submitted_tasks"] or 0,
                    "resubmission_tasks": r["resubmission_tasks"] or 0,
    ...
    def get_student_submission_rates(self) -> List[Dict[str, Any]]:
        ...
        Strictly calculates completion rate based on teacher-approved Completed tasks.
        """
        conn = get_connection(self.db_path)
        cursor = conn.cursor()
        try:
