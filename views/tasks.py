...
Implements reusable CRUD, multi-filtering, search, sorting, role-based completion workflow, and task modal dialog.
...
class TaskModalDialog(tk.Toplevel):
    ...
    def _create_form(self) -> None:
        ...
        # Status (Role-restricted options)
        ...
        if self.current_user.is_admin:
            status_choices = config.TASK_STATUSES
        else:
            # Student can set Pending, In Progress, Submitted
            status_choices = ["Pending", "In Progress", "Submitted"]
            if self.task and self.task.status in ("Needs Resubmission", "Completed"):
                status_choices.append(self.task.status)

        self.status_combo = ttk.Combobox(grid_row2, textvariable=self.status_var, values=status_choices, state="readonly", font=config.FONTS["body"])
        ...
        tk.Label(form_frame, text="Notes / Remarks / Teacher Feedback", font=config.FONTS["small_bold"], bg=config.COLORS["bg_card"], fg=config.COLORS["text_dark"]).pack(anchor="w", pady=(4, 2))
...
class TasksView(ttk.Frame):
    ...
    Enforces Role-Based Completion Workflow:
    - Student: Submit for Verification (Pending/In Progress/Needs Resubmission -> Submitted)
    - Admin: Review (Submitted -> Completed or Needs Resubmission)
    """
    def __init__(
        self,
        parent: tk.Widget,
    ...
    def _create_widgets(self) -> None:
        ...
        self.search_entry = ttk.Entry(toolbar, textvariable=self.search_var, width=16, font=config.FONTS["body"])
        ...
        self.subject_combo = ttk.Combobox(toolbar, textvariable=self.subject_filter_var, state="readonly", width=14, font=config.FONTS["small"])
        self.subject_combo.pack(side="left", padx=(0, 12))
        ...
        if not self.is_practicals_view:
            ...
            self.type_combo = ttk.Combobox(toolbar, textvariable=self.type_filter_var, values=type_options, state="readonly", width=11, font=config.FONTS["small"])
        ...
        self.status_combo = ttk.Combobox(toolbar, textvariable=self.status_filter_var, values=status_options, state="readonly", width=14, font=config.FONTS["small"])
        ...
        self.priority_combo.pack(side="left", padx=(0, 12))
        ...
        deadline_options = ["All Deadlines", "Overdue", "Due Today", "Upcoming", "Submitted", "Completed"]
        self.deadline_combo = ttk.Combobox(toolbar, textvariable=self.deadline_filter_var, values=deadline_options, state="readonly", width=11, font=config.FONTS["small"])
        self.deadline_combo.pack(side="left", padx=(0, 12))
        ...
        self.sort_combo = ttk.Combobox(toolbar, textvariable=self.sort_var, values=sort_options, state="readonly", width=15, font=config.FONTS["small"])
        ...
        self.tree.heading("status", text="Status / Verification")
        ...
        self.tree.tag_configure("submitted", background=config.COLORS["submitted_bg"])
        self.tree.tag_configure("resubmission", background=config.COLORS["resubmission_bg"])
        ...
        # Action Buttons (Differentiated by Student vs Administrator Role)
        btn_box = tk.Frame(control_bar, bg=config.COLORS["bg_main"])
        btn_box.pack(side="right")

        if self.user.is_student:
            # Student Action: Submit for Verification
            submit_btn = tk.Button(
                ...
                padx=12,
                pady=6,
                command=self._submit_selected_for_verification
            )
            submit_btn.pack(side="left", padx=(0, 8))
        else:
            # Admin Actions: Approve / Complete and Request Resubmission
            approve_btn = tk.Button(
                ...
                padx=10,
                pady=6,
                command=self._admin_approve_selected
            )
            ...
            reject_btn = tk.Button(
                ...
                padx=10,
                pady=6,
                command=self._admin_request_resubmission
            )
    ...
    def _submit_selected_for_verification(self) -> None:
        """Student submits task for teacher/admin verification."""
        task = self._get_selected_task()
        if not task:
            return

        if task.status == "Completed":
            messagebox.showinfo("Already Completed", "This task is already verified and marked as Completed.", parent=self)
            return
        if task.status == "Submitted":
            messagebox.showinfo("Awaiting Verification", "This task is already submitted and awaiting teacher/admin verification.", parent=self)
            return

        confirm_msg = f"Submit '{task.title}' for verification?"
        if task.status == "Needs Resubmission":
            confirm_msg = f"Resubmit updated '{task.title}' for teacher verification?"

        confirm = messagebox.askyesno("Confirm Submission", confirm_msg, parent=self)
        if confirm:
            success, msg = self.task_service.update_task_status(task.id, "Submitted", acting_user=self.user)
            if success:
                messagebox.showinfo("Submission Sent", "Task submitted successfully for verification! 📤", parent=self)
                self.refresh_data()
            else:
                messagebox.showerror("Error", msg, parent=self)

    def _admin_approve_selected(self) -> None:
        """Admin approves submitted task and marks it Completed."""
        task = self._get_selected_task()
        if not task:
            return
        if task.status == "Completed":
            messagebox.showinfo("Already Completed", "This task is already approved and marked as Completed.", parent=self)
            return

        confirm = messagebox.askyesno(
            "Approve Submission",
            f"Approve and mark '{task.title}' as Completed for student {task.student_name or 'student'}?",
            parent=self
        )
        if confirm:
            success, msg = self.task_service.update_task_status(task.id, "Completed", acting_user=self.user)
            if success:
                messagebox.showinfo("Approved", "Task approved and marked as Completed! ✅", parent=self)
                self.refresh_data()
            else:
                messagebox.showerror("Error", msg, parent=self)

    def _admin_request_resubmission(self) -> None:
        """Admin rejects submission and requests resubmission from student."""
        task = self._get_selected_task()
        if not task:
            return
        ...
        confirm = messagebox.askyesno(
            ...
            f"Request resubmission for '{task.title}' from student {task.student_name or 'student'}?",
            parent=self
        )
        if confirm:
            success, msg = self.task_service.update_task_status(task.id, "Needs Resubmission", acting_user=self.user)
            if success:
                messagebox.showinfo("Resubmission Requested", "Status updated to 'Needs Resubmission' ↻.", parent=self)
                self.refresh_data()
            else:
                messagebox.showerror("Error", msg, parent=self)
    ...
    def refresh_data(self) -> None:
        ...
        for idx, t in enumerate(tasks):
            ...
            if t.status == "Completed":
                ...
                state_badge = f"📤 {format_display_date(t.due_date)} (Submitted)"
                status_badge = "📤 Submitted (Under Review)"
            elif t.status == "Needs Resubmission":
                tag_list.append("resubmission")
                status_badge = "⚠️ Needs Resubmission"
