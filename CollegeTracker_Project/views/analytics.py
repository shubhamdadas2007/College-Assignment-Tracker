...
class AnalyticsView(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        sub_text = (
            "Visual insights on your task completion rates, deadline distribution, and subject workloads"
            if self.user.is_student else
            "Institutional overview of student completion rates, subject task distribution, and bottlenecks"
        )
        tk.Label(
            title_box,
            text=sub_text,
    ...
    def refresh_data(self) -> None:
        ...
        # 1. Chart 1: Task Status Distribution
        ...
        fig_completion = plot_subject_completion_rates(
            workloads=workloads,
            title="Subject Completion Rate (%)"
        )
        self._embed_figure(fig_completion, row=1, col=0)

        # 4. Chart 4 (Admin): Student Submission Progress (%)
        if self.user.is_admin:
            ...
            fig_student = plot_student_submission_rates(
                ...
                title="Student Submission Progress (%)"
            )
            ...
            tk.Label(
                info_card,
                text="🎯 Academic Progress Summary",
            ...
            tk.Label(
                info_card,
                text=f"Overall Completion: {overall_rate}%",
            ...
            summary_msg = (
                f"You have completed {completed} out of {total} assigned academic tasks. "
                "Keep submitting your practicals and assignments before due dates to maintain top grades!"
            )
            tk.Label(
                info_card,
                text=summary_msg,
