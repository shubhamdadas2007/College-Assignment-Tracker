...
Displays personal metrics, KPI cards, and upcoming deadline list (PRD Section 9).
"""
...
class StudentDashboardView(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        self.dl_tree.heading("due_date", text="Due Date / State")
        self.dl_tree.heading("priority", text="Priority")
        self.dl_tree.heading("status", text="Status")
        self.dl_tree.heading("actions", text="Status Action")

        self.dl_tree.column("title", width=190, anchor="w")
        self.dl_tree.column("subject", width=140, anchor="w")
        self.dl_tree.column("type", width=90, anchor="center")
        self.dl_tree.column("due_date", width=120, anchor="center")
        self.dl_tree.column("priority", width=80, anchor="center")
        self.dl_tree.column("status", width=90, anchor="center")
        ...
        tk.Label(
            tip_box,
            text="Marking practicals and journals as completed updates your live analytics instantly!",
    ...
    def refresh_data(self) -> None:
        ...
        if not upcoming_tasks:
            self.dl_tree.insert("", "end", values=("No pending tasks found! 🎉", "-", "-", "-", "-", "-", "-"))
            return
        ...
        for idx, task in enumerate(upcoming_tasks):
            ...
            status_icon = "⏳ Pending" if task.status == "Pending" else ("⚙️ In Progress" if task.status == "In Progress" else "✅ Completed")

            item_id = self.dl_tree.insert(
                "",
                "end",
                iid=str(task.id),
                values=(
                ...
                tags=(tag, task.deadline_state.lower().replace(" ", "_"))
            )
