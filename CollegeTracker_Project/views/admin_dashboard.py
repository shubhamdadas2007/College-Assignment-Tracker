...
Provides college-wide metrics, overdue student alerts, and subject workload breakdown (PRD Section 10).
"""
...
class AdminDashboardView(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        sub_lbl = tk.Label(
            welcome_box,
            text=f"Logged in as {self.user.name} ({self.user.username})  •  Organization Level Oversight",
        ...
        cards_config = [
            ...
            ("pending", "Pending Work", "⏳", config.COLORS["info"], config.COLORS["info_bg"]),
            ("completed", "Completed", "✅", config.COLORS["success"], config.COLORS["success_bg"]),
        ...
        for idx, (key, title, icon, color, bg_color) in enumerate(cards_config):
            ...
            lbl_icon = tk.Label(top_row, text=icon, font=("Segoe UI", 12), bg=config.COLORS["bg_card"])
            lbl_icon.pack(side="right")

            val_label = tk.Label(card, text="0", font=("Segoe UI", 20, "bold"), fg=color, bg=config.COLORS["bg_card"])
        ...
        columns_wl = ("subject", "total", "completed", "pending", "rate")
        ...
        self.wl_tree.heading("total", text="Total Tasks")
        self.wl_tree.heading("completed", text="Completed")
        self.wl_tree.heading("pending", text="Pending")
        self.wl_tree.heading("rate", text="Completion %")

        self.wl_tree.column("subject", width=160, anchor="w")
        self.wl_tree.column("total", width=70, anchor="center")
        self.wl_tree.column("completed", width=75, anchor="center")
        self.wl_tree.column("pending", width=70, anchor="center")
        self.wl_tree.column("rate", width=85, anchor="center")
    ...
    def refresh_data(self) -> None:
        ...
        self.kpi_cards["pending"].config(text=str(metrics.get("pending", 0)))
        ...
        if not workloads:
            self.wl_tree.insert("", "end", values=("No subjects found.", "0", "0", "0", "0%"))
        else:
