...
Generates live data visualization figures for Student and Administrator analytics.
...
def plot_task_status_distribution(metrics: Dict[str, Any], title: str = "Task Status Distribution") -> Figure:
    ...
    # Note: Overdue tasks are technically pending/in progress, so let's show remaining non-overdue pending + in_progress + completed + overdue
    # To avoid double-counting in chart:
    # Overdue tasks are deducted from pending/in_progress or shown distinctly
    total = completed + pending + in_progress
    if total == 0:
        return create_empty_figure(title)

    # Let's categorize slices clearly:
    # Active Pending, Active In Progress, Overdue, Completed
    # Overdue items are a subset of unfinished items
...
def plot_subject_task_counts(workloads: List[Dict[str, Any]], title: str = "Subject Task Workload") -> Figure:
    ...
    """
    # Filter only subjects with total_tasks > 0 or all subjects if list is small
    valid_data = [w for w in workloads if w["total_tasks"] > 0]
    if not valid_data:
        # If all subjects have 0 tasks, display empty figure
        return create_empty_figure(title)
...
def plot_student_submission_rates(student_rates: List[Dict[str, Any]], title: str = "Student Submission Progress (%)") -> Figure:
    """
    Chart 4 (Admin): Horizontal bar chart showing completion rates across students.
    """
